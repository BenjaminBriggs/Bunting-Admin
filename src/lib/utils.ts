...
export function validateKey(key: string): { valid: boolean; error?: string } {
	...
	if (!/^[a-z][a-z0-9_]*(?:\/[a-z][a-z0-9_]*)*$/.test(key)) {
		return {
			valid: false,
			error:
				'Flag key must start with a letter and contain only lowercase letters, numbers, underscores, and forward slashes for namespacing',
		};
	}
	...
	for (const segment of segments) {
		if (segment.length > 50) {
			return {
				valid: false,
				error: 'Flag key segments must be 50 characters or less',
			};
		}
		if (segment.endsWith('_')) {
			return {
				valid: false,
				error: 'Flag key segments cannot end with underscore',
			};
		}
	}
