...
export function resolveProtectionScheme(env: Env = process.env): ProtectionScheme {
...
export async function unprotectPrivateKey(
  ...
  if (envelope.scheme === 'local') {
    if (config.scheme !== 'local') {
      throw new Error('Stored key uses the local scheme but no SIGNING_KEY_SECRET is configured.');
    }
  ...
  if (envelope.scheme === 'kms') {
    if (config.scheme !== 'kms') {
      throw new Error('Stored key uses the KMS scheme but KMS is not configured.');
    }
    const plaintext = await config.kms.decrypt(Buffer.from(envelope.ciphertext, 'base64'));
    return plaintext.toString('utf8');
  }
 ...
 */
export async function loadPrivateKey(stored: string, env: Env = process.env): Promise<string> {
...
export async function storePrivateKey(pem: string, env: Env = process.env): Promise<string> {
...
export async function getProtectionConfig(env: Env = process.env): Promise<ProtectionConfig> {
  const scheme = resolveProtectionScheme(env);
  if (scheme.scheme === 'local') return scheme;
  ...
  const client = new KMSClient({ region: env.AWS_REGION || env.S3_REGION || 'us-east-1' });
