...
export default function SetupPage() {
  ...
  const handleNameChange = (name: string) => {
    ...
    setSetupData(prev => {
      const newArtifactUrl = generateArtifactUrl(prev.storageType, prev.storageConfig, identifier);
      return {
        ...prev,
        appName: name,
  ...
  const generateArtifactUrl = (type: 'minio' | 'aws' | 'r2' | 'b2' | 'custom', config: any, appIdentifier: string) => {
  ...
  const handleStorageTypeChange = (type: 'minio' | 'aws' | 'r2' | 'b2' | 'custom') => {
    let defaultConfig = {
      bucket: '',
      region: 'us-east-1',
      endpoint: '',
    ...
    const newArtifactUrl = generateArtifactUrl(type, defaultConfig, setupData.appIdentifier);
  ...
  const handleCreate = async () => {
    try {
      ...
      const appData = {
        ...
        artifactUrl: setupData.artifactUrl || `https://cdn.example.com/configs/${setupData.appIdentifier}/`,
        publicKeys: [
          {
            kid: 'default',
        ...
        return (
          <Stack spacing={3}>
            ...
            <TextField
              ...
              onChange={(e) => setSetupData(prev => ({ ...prev, appIdentifier: e.target.value }))}
              fullWidth
              required
            ...
            <TextField
              ...
              onChange={(e) => setSetupData(prev => ({ ...prev, artifactUrl: e.target.value }))}
              fullWidth
          ...
          <Stack spacing={3}>
            ...
            <Stack direction="row" spacing={2} sx={{ flexWrap: 'wrap', gap: 1 }}>
              {process.env.NODE_ENV === 'development' && (
                <Button
                  variant={setupData.storageType === 'minio' ? 'contained' : 'outlined'}
              ...
              <Button
                variant={setupData.storageType === 'aws' ? 'contained' : 'outlined'}
              ...
              <Button
                variant={setupData.storageType === 'r2' ? 'contained' : 'outlined'}
              ...
              <Button
                variant={setupData.storageType === 'b2' ? 'contained' : 'outlined'}
              ...
              <Button
                variant={setupData.storageType === 'custom' ? 'contained' : 'outlined'}
            ...
            {setupData.storageType === 'r2' && (
              <Alert severity="info">
                <strong>Cloudflare R2 Configuration</strong><br/>
                Use your Cloudflare account ID as the region. Get API tokens from the Cloudflare dashboard.
              </Alert>
            )}

            {setupData.storageType === 'b2' && (
              <Alert severity="info">
                <strong>Backblaze B2 Configuration</strong><br/>
                Create an application key in your Backblaze B2 console. Use your application key ID and key.
              </Alert>
            )}

            {setupData.storageType === 'custom' && (
              <Alert severity="info">
                <strong>Custom S3-Compatible Storage</strong><br/>
                For any S3-compatible storage provider. Enter the full endpoint URL and credentials.
              </Alert>
            )}
            ...
            <TextField
              ...
              onChange={(e) => setSetupData(prev => {
                const newConfig = { ...prev.storageConfig, bucket: e.target.value };
                const newArtifactUrl = generateArtifactUrl(prev.storageType, newConfig, prev.appIdentifier);
                return {
                  ...prev,
            ...
            <TextField
              ...
              onChange={(e) => setSetupData(prev => {
                const newConfig = { ...prev.storageConfig, region: e.target.value };
                const newArtifactUrl = generateArtifactUrl(prev.storageType, newConfig, prev.appIdentifier);
                return {
                  ...prev,
                  storageConfig: newConfig,
            ...
            {(setupData.storageType === 'minio' || setupData.storageType === 'b2' || setupData.storageType === 'custom') && (
              <TextField
                label="Endpoint"
                value={setupData.storageConfig.endpoint}
                onChange={(e) => setSetupData(prev => {
                  const newConfig = { ...prev.storageConfig, endpoint: e.target.value };
                  const newArtifactUrl = generateArtifactUrl(prev.storageType, newConfig, prev.appIdentifier);
                  return {
                    ...prev,
                    storageConfig: newConfig,
            ...
            {setupData.storageType !== 'minio' && (
              <>
                <TextField
                  ...
                  onChange={(e) => setSetupData(prev => ({
                    ...prev,
                    storageConfig: { ...prev.storageConfig, accessKeyId: e.target.value }
                  }))}
                  fullWidth
                ...
                <TextField
                  ...
                  onChange={(e) => setSetupData(prev => ({
                    ...prev,
                    storageConfig: { ...prev.storageConfig, secretAccessKey: e.target.value }
                  }))}
                  fullWidth
                  helperText="Leave empty to use environment variables or IAM roles"
                />
          ...
          <Stack spacing={3}>
            ...
            <Card variant="outlined">
              <CardContent>
                ...
                <Typography variant="body2" color="text.secondary">ID: {setupData.appIdentifier}</Typography>
              ...
              <CardContent>
                ...
                <Stack direction="row" spacing={1} sx={{ mb: 1 }}>
                  <Chip 
                    ...
                    size="small" 
                    color={setupData.storageType === 'minio' ? 'secondary' : 'primary'} 
                  />
                </Stack>
                <Typography variant="body2">Bucket: {setupData.storageConfig.bucket}</Typography>
                <Typography variant="body2">Region: {setupData.storageConfig.region}</Typography>
                {setupData.storageConfig.endpoint && (
                  <Typography variant="body2">Endpoint: {setupData.storageConfig.endpoint}</Typography>
                )}
              </CardContent>
            </Card>
            ...
            <Card variant="outlined">
              <CardContent>
                <Typography variant="subtitle2" color="text.secondary">Fetch Policy</Typography>
                <Typography variant="body2">
                  Minimum interval: {setupData.fetchPolicy.minIntervalHours} hours
                </Typography>
                <Typography variant="body2">
    ...
    <Box sx={{ 
      ...
      <Container maxWidth="md">
        <Box sx={{ textAlign: 'center', mb: 4 }}>
          <Flag sx={{ fontSize: 80, color: 'primary.main', mb: 2 }} />
          <Typography variant="h2" component="h1" sx={{ mb: 1, fontWeight: 600 }}>
            Welcome to Bunting
          </Typography>
          <Typography variant="h5" color="text.secondary" sx={{ mb: 1 }}>
            Self-hosted feature flags for Apple platforms
          </Typography>
