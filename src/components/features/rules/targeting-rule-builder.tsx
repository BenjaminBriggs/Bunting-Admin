...
export function TargetingRuleBuilder({
	...
	const hasEmptyConditions = rule.conditions.some(
		(condition) =>
					...
					<Stack spacing={3}>
						...
						<Box>
								...
								{rule.conditions.length > 1 && (
									<FormControl size="small" sx={{ minWidth: 100 }}>
										...
										<Select
											...
											label="Logic"
											onChange={(e) =>
												onChange({ ...rule, conditionLogic: e.target.value })
											}
										>
							...
							{rule.conditions.length === 0 ? (
								<Alert severity="error" sx={{ mb: 2 }}>
									No conditions defined. Every rule must have at least one
									condition. Please add a condition or delete this rule.
								</Alert>
							) : (
								<Stack spacing={2}>
						...
						{hasEmptyConditions && (
							<Alert severity="warning" icon={<Warning />}>
								Some conditions are incomplete. Please fill in all condition
								values.
							</Alert>
						)}
					</Stack>
