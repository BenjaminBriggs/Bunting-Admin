...
import { Add, CheckCircle, Info, Warning } from '@mui/icons-material';
import {
	Alert,
	Box,
	Button,
...
import type { CohortTargetingRule, RuleValidationError } from '@/types/rules';
...
export function CohortRulesContainer({
	rules,
	onChange,
	appId,
}: CohortRulesContainerProps) {
	...
	const handleRuleChange = (
		ruleId: string,
		updatedRule: CohortTargetingRule,
	) => {
		...
		<Card>
			<CardContent sx={{ p: 3 }}>
				...
				<Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
					Define the conditions that determine which users belong to this
					cohort. Cohorts are reusable condition groups that can be referenced
					in tests and rollouts.
				</Typography>
				...
				<Stack spacing={3}>
					...
					{rules.length === 0 ? (
						<Alert severity="info" icon={<Info />}>
							...
							<br />
							Add rules to include specific users based on their attributes
							(e.g., iOS 18 users, beta testers).
						</Alert>
					) : (
						<Stack spacing={2}>
							<Alert severity="info">
								Users matching ANY of these rules will be included in this
								cohort. Cohorts can then be used in tests and rollouts for more
								complex targeting.
							</Alert>
							...
							{rules.map((rule, index) => (
								<CohortRuleBuilder
									...
									onChange={(updatedRule) =>
										handleRuleChange(rule.id, updatedRule)
									}
					...
					<Alert severity="info">
						<Typography variant="body2">
							...
							<br />
							• Rollouts - For percentage-based feature releases to targeted
							users
							<br />• Flag Rules - For conditional flag values based on user
							attributes
						</Typography>
					</Alert>
