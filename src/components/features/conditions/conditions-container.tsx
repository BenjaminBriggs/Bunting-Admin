			...
			<Stack spacing={2}>
				{conditions.length === 0 ? (
					...
					<>
						{conditions.length > 1 && (
							<Alert severity="info">
								All conditions must be met (AND logic). Users will only match if
								they satisfy every condition below.
							</Alert>
						)}
			...
			{conditions.length > 0 && (
				<Paper variant="outlined" sx={{ mt: 3, p: 2, bgcolor: 'grey.50' }}>
					<Typography variant="subtitle2" sx={{ mb: 1 }}>
						Summary: {conditions.length} condition
						{conditions.length === 1 ? '' : 's'} defined
					</Typography>
					<Typography variant="caption" color="text.secondary">
						{hasErrors
							? 'Some conditions need attention before they can be saved.'
							: hasWarnings
								? 'Conditions are valid but have potential issues.'
