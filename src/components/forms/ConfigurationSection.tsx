...
export function ConfigurationSection({
	...
	const content = (
		<Stack spacing={2}>
			...
			<Collapse in={expanded || !collapsible}>{renderContent()}</Collapse>
		</Stack>
	);
...
export function SettingsSection(
	props: Omit<ConfigurationSectionProps, 'variant'>,
) {
	return <ConfigurationSection {...props} variant="outlined" />;
}

export function SetupSection(
	props: Omit<ConfigurationSectionProps, 'variant' | 'collapsible'>,
) {
	return (
		<ConfigurationSection {...props} variant="elevated" collapsible={false} />
	);
}
...
export function ConfigurationGroup({
	...
	switch (variant) {
		...
		case 'paper':
			return <Paper sx={{ p: 4 }}>{content}</Paper>;
