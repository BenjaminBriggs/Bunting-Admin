...
function migrateCondition(condition: any): { condition: any; changed: boolean } {
  let changed = false;
...
function migrateConditionArray(conditions: any[]): { conditions: any[]; changed: boolean } {
...
function migrateVariantsJson(variantsJson: any): { variants: any; changed: boolean } {
  ...
  for (const env of ['development', 'staging', 'production']) {
    if (Array.isArray(updated[env])) {
      updated[env] = updated[env].map((variant: any) => {
        if (Array.isArray(variant.conditions) && variant.conditions.length > 0) {
          const { conditions, changed } = migrateConditionArray(variant.conditions);
          if (changed) {
            anyChanged = true;
...
async function migrateCohorts() {
  ...
  for (const cohort of cohorts) {
    const conditions = Array.isArray(cohort.conditions) ? cohort.conditions : [];
    const { conditions: migrated, changed } = migrateConditionArray(conditions as any[]);
    if (changed) {
      await prisma.cohort.update({ where: { id: cohort.id }, data: { conditions: migrated } });
      updated++;
    }
  }
...
async function migrateTestRollouts() {
  ...
  for (const tr of testRollouts) {
    ...
    const { conditions: migrated, changed } = migrateConditionArray(conditions as any[]);
    if (changed) {
      await prisma.testRollout.update({ where: { id: tr.id }, data: { conditions: migrated } });
      updated++;
    }
  }
