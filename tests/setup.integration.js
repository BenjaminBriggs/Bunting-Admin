...
beforeAll(async () => {
	...
	try {
		...
		process.env.DATABASE_URL =
			process.env.DATABASE_URL ||
			'postgresql://postgres:postgres@127.0.0.1:5432/bunting_test';
...
beforeAll(() => {
	console.error = (...args) => {
		...
		if (
			!message.includes('Warning: Could not clean database') &&
...
expect.extend({
	...
	toBeValidISO8601Date(received) {
		...
		const pass =
			date.toString() !== 'Invalid Date' && date.toISOString() === received;
	...
	toMatchVersionPattern(received) {
		...
		if (pass) {
			return {
				message: () =>
					`expected ${received} not to match version pattern YYYY-MM-DD.N`,
				pass: true,
			};
		}
		...
		return {
			message: () =>
				`expected ${received} to match version pattern YYYY-MM-DD.N`,
			pass: false,
		};
	},
});
