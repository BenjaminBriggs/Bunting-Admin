...
describe('signDetached', () => {
  const config = JSON.stringify({ schema_version: 1, flags: { a: { type: 'bool' } } }, null, 2);
  let key: CryptoKey;

  beforeAll(async () => {
    key = (await importPKCS8(privateKey, 'RS256')) as CryptoKey;
  });
  ...
  it('produces a compact detached JWS (header..signature, empty payload segment)', async () => {
    ...
    expect(header).toMatchObject({ alg: 'RS256', kid: 'k1', b64: false, crit: ['b64'] });
  });
  ...
  it('fails verification if a single byte of the config changes', async () => {
    ...
    expect(sdkVerify(jws, config.replace('bool', 'int'), publicKey)).toBe(false);
  });
});
