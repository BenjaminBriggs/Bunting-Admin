...
describe('artifactUrlFor', () => {
  it('derives a directory URL from CDN_BASE_URL', () => {
    ...
    expect(artifactUrlFor('com.acme.app', env)).toBe('https://cdn.example.com/com.acme.app/');
  });
  ...
  it('falls back to S3 endpoint + bucket when no CDN is set (MinIO/custom)', () => {
    const env = { S3_ENDPOINT: 'http://localhost:9000', S3_BUCKET: 'bunting-configs' };
    expect(artifactUrlFor('demo', env)).toBe('http://localhost:9000/bunting-configs/demo/');
  });
  ...
  it('returns empty string for an empty identifier', () => {
    expect(artifactUrlFor('', { CDN_BASE_URL: 'https://cdn.example.com' })).toBe('');
  });
});
...
describe('buildS3Config', () => {
  it('includes explicit credentials when both env keys are present (local/MinIO)', () => {
    ...
    expect(cfg.credentials).toEqual({ accessKeyId: 'admin', secretAccessKey: 'admin123' });
