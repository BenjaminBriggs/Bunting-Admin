...
describe('JSON Spec Functions', () => {
	...
	describe('Key Validation', () => {
		test('validates valid keys', () => {
			...
			expect(validateIdentifierKey('store_use_new_feature')).toEqual({
				valid: true,
			});
	...
	describe('Key Normalization', () => {
		test('normalizes display names to valid keys', () => {
			expect(normalizeToIdentifierKey('Store: Use New Feature')).toBe(
				'store_use_new_feature',
			);
			expect(normalizeToIdentifierKey('My Great Feature!')).toBe(
				'my_great_feature',
			);
