...
describe('validation-schemas', () => {
  describe('conditionSchema', () => {
    ...
    it('accepts every SDK operator', () => {
      const ops = [
        'equals', 'does_not_equals', 'in', 'not_in', 'greater_than',
        'less_than', 'greater_than_or_equal', 'less_than_or_equal',
        'between', 'custom',
      ];
      for (const operator of ops) {
        expect(conditionSchema.safeParse({ ...valid, operator }).success).toBe(true);
      }
    });
  });
  ...
  describe('createFlagSchema', () => {
    ...
    it('accepts the short-form wire types, not the SDK long form', () => {
      expect(createFlagSchema.safeParse({ ...valid, type: 'int' }).success).toBe(true);
      // 'boolean'/'integer' are the SDK *output* form, never accepted on the wire
      expect(createFlagSchema.safeParse({ ...valid, type: 'boolean' }).success).toBe(false);
      expect(createFlagSchema.safeParse({ ...valid, type: 'integer' }).success).toBe(false);
    });
    ...
    it('accepts json object/array flag values', () => {
      expect(createFlagSchema.safeParse({
        ...valid, type: 'json',
        defaultValues: { development: { a: 1 }, staging: [1, 2], production: {} },
      }).success).toBe(true);
    });
  });
  ...
  describe('createRolloutSchema', () => {
    ...
    it('rejects percentage out of range', () => {
      expect(createRolloutSchema.safeParse({ ...base, percentage: 150 }).success).toBe(false);
      expect(createRolloutSchema.safeParse({ ...base, percentage: -1 }).success).toBe(false);
    });

    it('accepts boundary percentages', () => {
      expect(createRolloutSchema.safeParse({ ...base, percentage: 0 }).success).toBe(true);
      expect(createRolloutSchema.safeParse({ ...base, percentage: 100 }).success).toBe(true);
    });
  });
  ...
  describe('createTestSchema', () => {
    it('rejects a non-integer variantCount', () => {
      const r = createTestSchema.safeParse({
        key: 'k', name: 'n', appId: 'a', conditions: [],
        variantCount: 2.5, trafficSplit: [50, 50], variantNames: ['a', 'b'],
      });
      expect(r.success).toBe(false);
    });
  });
  ...
  describe('createTestRolloutSchema (discriminated)', () => {
    ...
    it('accepts a valid ROLLOUT', () => {
      const r = createTestRolloutSchema.safeParse({
        appId: 'a', name: 'n', type: 'ROLLOUT', percentage: 25,
      });
  ...
  describe('update schemas strip unknown keys (mass-assignment guard)', () => {
    ...
    it('still enforces percentage bounds on rollout update', () => {
      expect(updateRolloutSchema.safeParse({ percentage: 101 }).success).toBe(false);
      expect(updateRolloutSchema.safeParse({ percentage: 50 }).success).toBe(true);
    });
  });
});
